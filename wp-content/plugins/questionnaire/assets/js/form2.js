$(function() {
    ...
    $('[name="q04_a12"]').on('click', function(){
        if ($(this).prop('checked') === true) {
            $(this).closest('fieldset').find('textarea').removeClass('hide');
        } else {
            $(this).closest('fieldset').find('textarea').val('').addClass('hide');
    ...
    $('.question_4 ').find('input').on('click', function(){
        if ($(this).prop('checked') === true) {
            $(this).closest('.row').find('label').css('text-decoration', 'line-through');
            $(this).closest('.row').find('label').css('color', '#B4B4B4');
        }
        else {
            $(this).closest('.row').find('label').css('text-decoration', 'none');
            $(this).closest('.row').find('label').css('color', '#333');
        }
    });

    $('[name="q04_a11"]').on('click', function(){
        $(this).closest('.row').find('label').css('text-decoration', 'none');
        $(this).closest('.row').find('label').css('color', '#333');

        if ($(this).prop('checked') === true) {
            $(this).closest('fieldset').find('input[name!="q04_a11"]').each(function(){
                $(this).closest('.row').find('label').css('text-decoration', 'none');
                $(this).closest('.row').find('label').css('color', '#B4B4B4');
                $(this).prop('checked', false).prop('disabled', true);
            });
            $(this).closest('fieldset').find('.other_answer').val('').addClass('hide');
        } else {
            $(this).closest('fieldset').find('input[name!="q04_a11"]').prop('disabled', false);
            $(this).closest('fieldset').find('input[name!="q04_a11"]').each(function(){
                $(this).closest('.row').find('label').css('text-decoration', 'none');
                $(this).closest('.row').find('label').css('color', '#333');
            });
    ...
    $('[name="button_submit"]').on('click', function(event){
        ...
        if (send_anket) {
            $.post(
                site_url+"/be/anketa_skill.php", 
                $("form").serialize(), 
                function(data){
                    if (data === 'true') {
                        window.location = site_url+'/successful-fill-skilled';
                    }
                }
            );
        }
        ...
        if (!succes) {
            ...
            fieldset.append(
                '<label class="row error_questions custom_error">'+
                    'Вычеркните хотя бы один вариант из списка (количество вариантов, которые вы можете вычеркнуть - неограничено)'+
                '</label>'
            );
        }
        else {
