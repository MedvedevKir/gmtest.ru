...
function sp_check_version() 
{
	...
	$file = wp_remote_get( sp_ssl_http() . 'splashingpixels.s3.amazonaws.com/versions.xml' );
			...
			foreach( $versions['versions']['item'] as $item ) 
			{
				...
				if ( $item['_attr']['name'] == strtolower( trim( $theme['Name'] ) ) ) 
				{
					...
					if ( $theme['Version'] < $cur ) 
					{
						...
						$notification .= '<p class="update">' . sprintf( __( 'There is a new version of your theme available (' . '%s' . '). Please visit', 'sp' ), $item['value'] ) . ' <a href="' . esc_url( 'http://splashingpixels.com/member-dashboard/' ) . '" title="Splashing Pixels" target="_blank">Splashing Pixels</a> to download. <a href="' . esc_url( 'http://splashingpixels.com/wp-content/themes/sp/versions/' . strtolower( trim( $spthemeinfo['Name'] ) ) . '_changelog.txt' ) . '" title="Theme Changelog" target="_blank">(ChangeLog)</a>(' . __( 'To disable this message permanently', 'sp' ) . ' <a href="#" class="hide_msg" title="Hide Message">' . __( 'click here', 'sp' ) . '</a>)</p>';
					}
				}
				if ( $item['_attr']['name'] == 'framework' ) 
				{
					...
					{
						// build the notification HTML to display above SP theme control panel
						$notification .= '<p class="update">' . sprintf( __( 'There is a new version of SP Framework available (' . '%s' . '). Please visit', 'sp' ), $item['value'] ) . ' <a href="' . esc_url( 'http://splashingpixels.com/member-dashboard/' ) . '" title="Splashing Pixels" target="_blank">Splashing Pixels</a> ' . __( 'to download.', 'sp' ) . ' <a href="' . esc_url( 'http://splashingpixels.com/wp-content/themes/sp/versions/framework_changelog.txt' ) . '" title="Theme Changelog" target="_blank">' . __( '(ChangeLog)', 'sp' ) . '</a> ' . __( '(To disable this message permanently', 'sp' ) . ' <a href="#" class="hide_msg" title="Hide Message">' . __( 'click here', 'sp' ) . '</a>)</p>';
					}
				}
