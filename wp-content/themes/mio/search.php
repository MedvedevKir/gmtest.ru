...
<?php if ( have_posts() ) : ?>
				<header class="page-header">
					<h1 class="page-title"><?php printf( __( 'Результат поиска: %s', 'sp' ), '<span>' . get_search_query() . '</span>' ); ?></h1>
                </header>
				  <?php
...
<?php else : ?>
				<article id="post-0" class="post no-results not-found">
					<header class="entry-header">
						<h1 class="entry-title"><?php _e( 'Результаты поиска', 'sp' ); ?></h1>
					</header><!-- .entry-header -->

					<div class="entry-content">
						<p><?php _e( 'К сожалению, ничего не найдено', 'sp' ); ?></p>
					</div><!-- .entry-content -->
				</article><!-- #post-0 -->
<?php endif; ?>
			</div><!-- #content -->
